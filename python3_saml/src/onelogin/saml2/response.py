...
from python3_saml.src.onelogin.saml2.utils import OneLogin_Saml2_Utils, OneLogin_Saml2_Error, OneLogin_Saml2_ValidationError, return_false_on_exception
from python3_saml.src.onelogin.saml2.xml_utils import OneLogin_Saml2_XML
...
class OneLogin_Saml2_Response(object):
    ...
    def is_valid(self, request_data, request_id=None, raise_exceptions=False):
        ...
        try:
            ...
            if self.__settings.is_strict():
                ...
                if security['failOnAuthnContextMismatch'] and requested_authn_contexts and requested_authn_contexts is not True:
                    ...
                    with open("/var/log/fairdata-sso-onelogin.log", "a") as errlog:
                        errlog.write("authn_contexts" + str(authn_contexts) + "\n")
                        errlog.write("requested_authn_contexts" + str(requested_authn_contexts) +  "\n")
                        errlog.close()
            ...
            # log.info('Insert SSO app logging to SSO log file here')

            # This logs to a separate file
            with open("/var/log/fairdata-sso-onelogin.log", "a") as errlog:
                errlog.write("Exception error: " + str(err) + "\n")
                errlog.close()
